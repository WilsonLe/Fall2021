...
the MST in O(1). The memory efficiency will not be impacted much, since we have the maximum of O(E) vertices in the priority queue, 
while we already using O(V + E) memory to store graph. The time complexity will be O(E log(E)), but since E = O(V^2) => log(E) = O(log(V^2)) = O(logV).
Therefore, the time complexity will be O(E logV) 
*/
